...
int ab_tag_create(attr attribs, plc_tag_p *ptag)
{
    int rc = PLCTAG_STATUS_OK;
    
    pdebug(DEBUG_INFO, "Starting.");
    
    /* split out the different types of tag. */
    switch(get_plc_type(attribs)) {
        case LOGIX:
            rc = logix_tag_create(attribs, ptag);
            break;
        
        case MICRO800:
            pdebug(DEBUG_WARN, "Unknown type of PLC!");
            *ptag = NULL;
...
plc_type_t get_plc_type(attr attribs)
{
    ...
    if(str_cmp_i(plc,"logix") == 0 || str_cmp_i(plc,"lgx") == 0 || str_cmp_i(plc,"controllogix") == 0 || str_cmp_i(plc,"compactlogix") == 0) {
        return LOGIX;
    } else if(str_cmp_i(plc, "micro800") == 0) {
        return MICRO800;
    } else if(str_cmp_i(plc, "micrologix") == 0 || str_cmp_i(plc, "mlogix") == 0 || str_cmp_i(plc,"mlgx") == 0) {
        return MICROLOGIX;
    } else if(str_cmp_i(plc, "slc") == 0 || str_cmp_i(plc, "slc500") == 0) {
        return SLC;
    } else if(str_cmp_i(plc, "plc") == 0 || str_cmp_i(plc, "plc5") == 0) {
        /* determine whether this is via DHP or not. */
        const char *dhp_conn = attr_get_str(attribs,"plc5_dhp_conn", "NONE");

        if(str_cmp_i(dhp_conn,"NONE") == 0) {
            return PLC5;
        } else {
            return PLC5_DHP;
        }
...
//
//
//plc_tag_p ab_tag_create_old(attr attribs)
//{
