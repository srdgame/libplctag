...
std::mutex plcMapMutex;
std::map<std::string, std::weak_ptr<LogixPLC> > plcMap;

static std::vector<std::string> stringSplit(std::string input, char separator) 
{
    std::vector<std::string> strings;
    std::istringstream inputStream(input);
    std::string s;    
    while(getline(inputStream, s, separator)) {
        strings.push_back(s);
    }
...
std::shared_ptr<LogixPLC> LogixPLC::Get(attr attribs)
{
    int rc = PLCTAG_STATUS_PENDING;
    std::shared_ptr<LogixPLC> plc = nullptr;
    
    pdebug(DEBUG_INFO, "Starting.");
    
    
    std::string path = attr_get_str(attribs, "path", "");
    
    if(path.size() == 0) {
        pdebug(DEBUG_WARN, "Unable to get path!");
        rc = PLCTAG_ERR_BAD_GATEWAY;
    } else {
        std::lock_guard<std::mutex> guard(plcMapMutex);

        std::map<std::string, std::weak_ptr<LogixPLC> >::iterator it = plcMap.find(path);
        
        if(it != plcMap.end()) {
            plc = it->second.lock();
        } else {
            plc = std::make_shared<LogixPLC>(path);
...
LogixPLC::LogixPLC(std::string path)
{
    pdebug(DEBUG_INFO, "Starting.");
    ...
    this->state = START;
    this->setStatus(PLCTAG_STATUS_PENDING);    
    this->plcThread = std::thread(LogixPLC::run, this);
...
int LogixPLC::setStatus(int newStatus)
{
...
const char *LogixPLC::decodeCIPErrorShort(uint8_t *data)
{
	int index = 0;
	int primary_code = 0;
...
const char *LogixPLC::decodeCIPErrorLong(uint8_t *data)
{
	int index = 0;
	int primary_code = 0;
	int secondary_code = 0;
...
int LogixPLC::decodeCIPErrorCode(uint8_t *data)
{
	int index = 0;
	int primary_code = 0;
	int secondary_code = 0;
