...
static struct error_code_entry error_code_table[] = {
    ...
    {0x06, 0, PLCTAG_ERR_TOO_LONG, "Data requested would not fit in response packet.", "The data to be read/written needs to be broken up into multiple packets.0x070000 Connection lost: The messaging connection was lost."},
...
const char *decode_cip_error_str(uint8_t primary_code, uint32_t secondary_code, int short_or_long)
{
    int index = 0;
...
int decode_cip_error_status(uint8_t primary_code, uint32_t secondary_code)
{
    int index = 0;
    const char *result = NULL;
